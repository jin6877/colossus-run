...
> ### ⚠️ 코어 오버라이드 v2 (라이브 플레이 피드백, 2026-07-24)
> 직전 오버라이드(반전 카메라 + 50m 타이탄 + 발 스톰프)를 플레이 후 **다시 전환**했다. "그냥 달려도 안 죽는다 / 거인이 너무 크다 / 앞이 안 보인다"를 잡는다. 아래가 **현행 코어**다:
> - **파수꾼 50m → ~4m 날렵한 포식자**: 압도적 거인이 아니라 사람 좀 넘는 크기의 **빠르고 민첩한 도자기 포식자**. 도자기 균열·냉광·가면 디테일은 유지하되 비율/실루엣을 4m 이족 스토커로 재조정(50m 왜곡 비율 폐기). 위협은 크기가 아니라 **속도·공격성**. (`lib/render/wardenRig.ts` 4m 재작성, `WARDEN_HEIGHT=4`.)
> - **핵심 공격 = 손 할퀴기(claw swipe)**: 발 스톰프를 코어에서 내리고 **팔을 뒤로 당겼다 앞으로 긁는 할퀴기**로. 파수꾼이 주인공 발뒤꿈치까지 러버밴드로 바짝 붙고(FOLLOW_GAP≈4.2m), 주기적으로 **차선 band를 lock → 바닥 rake 텔레그래프(색칠 금지) → strike**. strike 순간 그 band(±`SWIPE_HALF`≈2.5m) 안에 있으면 사망. **가만히 직진하면 band를 못 벗어나 죽는다**(fail pressure의 핵심). 좌우 juke로 회피(+저/고 rake는 점프/슬라이드). (`lib/warden.ts`, `lib/render/clawTelegraph.ts`, engine `resolveSwipe`.)
> - **카메라 = 3/4 오버숄더 후방**: 다시 등 뒤에서 **전방(도망칠 길)을 넉넉히** 본다. 주인공은 화면 하단/뒤, 앞 도로에 여백, 발뒤꿈치의 4m 포식자는 프레임에 남는다. steer 부호 반전 제거(화면 기준 직관 복구). (`lib/chaseCamera.ts`.)
> - **전방 장애물 복원**: 차량·잔해 블록·틈(gap)·낮은 바리케이드·머리 위 바를 **형태+자연 그림자로만** 읽히게 재생성(형광/붉은 색칠 금지). 못 피하면 graze(감속)→포식자 접근→할퀴기 사망, 틈은 즉사. → **"앞 장애물 회피 + 뒤 할퀴기 회피"가 이중 긴장**. (`lib/chunk/chunkBuild.ts` 생성, engine `checkObstacles`.)
> - **파괴 4m 스케일**: 건물 토플 대신 파수꾼이 질주하며 **먼지를 차고(stride dust) 저층(h<14m) 구조물만** 헤쳐 부순다(타워 토플 없음). Ashen Dusk 톤·파괴 먼지 스펙터클은 유지.
> - **유지**: 디테일 도자기 포식자, Ashen Dusk 톤, 생존거리 점수·결과카드·시드 재현, 무상태.
>
> 아래 §7(추격 카메라)의 수치는 이 오버라이드로 **대체**(chaseCamera.ts가 계약). §3~§5의 파수꾼 크기(50m)·발 스톰프·"전방 장애물 제거" 서술도 이 v2가 우선. 나머지(스트리밍/톤/무상태/파괴엔진)는 유효.

> ### ⚠️ 코어 오버라이드 v3 (라이브 피드백, 2026-07-24)
> 플레이 후 두 가지를 전환. **(a) 파수꾼 = 살아있는 유기 생물**: v2 의 도자기 판+냉광 seam+가면이 로봇처럼 보여, 살·근육·힘줄·가죽 + 진짜 발톱 + 벌어지는 아가리/이빨 + 앞을 향한 눈(따뜻한 eyeshine head look-at) + 호흡으로 재설계. 4m·빠름·IK 보행·손 할퀴기·치명성은 유지. **(b) 카메라 = 정면 반전(giant-frontal) 복귀**: v2 의 3/4 오버숄더에서 다시 **주인공 앞에서 뒤를 보는 반전 카메라**로. 주인공이 렌즈로 도망치고 4m 포식자가 정면에서 덮친다. 전방 장애물이 화면 하단 전경에 보이도록 카메라를 뒤/위로 빼 반응시간 확보, 좌우 입력은 화면 기준(엔진 steer flip). 냉광 시안 signature 폐기(조직 발광은 따뜻). 상세는 **DESIGN.md 오버라이드 v3 가 계약**. `chaseCamera.ts`·`wardenRig.ts`·`constants.ts` 갱신.
